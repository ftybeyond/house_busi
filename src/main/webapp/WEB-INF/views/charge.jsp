			...
			<div class="row">
				...
				<div class="col-md-9">
					...
					<div class="box box-danger row">
						...
						<!-- /.box-header -->
						<div class="box-body">
							<table id="chargeList" class="table table-bordered table-hover">
								<thead>
								<tr>
									<th>合同编码</th>
									<th>门牌地址</th>
...
<div id="popWin" style="display: none;">
		<div class="table-responsive">
			<table class="table">
				...
				<tr>
                    <th>缴费票号：</th>
                    <td><input type="text" class="form-control" name="invoice"/></td>
				</tr>
				<tr>
					<th>缴费时间：</th>
					<td><input type="text" class="form-control date" name="chargeDate" placeholder="缴费时间"></textarea>
					</td>
				</tr>
				<tr>
					<th>备注信息：</th>
...
<script type="text/javascript">
	var popWin;
	var doCharge = function(row){
        var rowData = $("#chargeList").DataTable().row(row).data();
        var payment = $("#popWin input[name='payment']").val(rowData.payable);
        popWin = layer.open({
            type: 1,
            title: false, //不显示标题
            ...
            btn: ["确定", "取消"],
			btn1:function(){
                var payment = $("#popWin input[name='payment']").val();
                var invoice = $("#popWin input[name='invoice']").val();
                var remark = $("#popWin textarea[name='remark']").val();
                var chargeDate = $("#popWin input[name='chargeDate']").val();
                if(!payment||payment.length <1){
                    layer.alert("请输入缴费金额！");
                    return;
				}
                if(!chargeDate||chargeDate.length <1){
                    layer.alert("请输入缴费时间！");
                    return;
                }
				$.post("<%=basePath%>rest/charge/do",{id:rowData.id,payment:payment,invoice:invoice,chargeDate:chargeDate,remark:remark},null,"json").done(function(rsp){
                    if(rsp.resultCode =="0000"){
                        layer.alert(rsp.description,{title:"操作成功",icon:1});
                        $("#chargeList").DataTable().ajax.reload();
                        layer.close(popWin);
                    }else{
    ...
    $(document).ready(function(){
        loadDeps(["<%=basePath%>dictionary/AgreementState.json"],function(loadedData) {
            ...
            var table = $('#chargeList').DataTable({
                ...
                columnDefs: [ {
                    ...
                    "render": function ( data, type, row, meta ) {
                        var btns = '<button type="button" class="btn btn-danger btn-xs" onclick="doCharge('+meta.row+')" >缴费</button>';
                        return btns;
                    }
                } ]
            });
